...
推送 `v*` 版本标签时，GitHub Actions 会构建 `linux/amd64`、`linux/arm64`
镜像并发布到 GitHub Container Registry（也可手动 `workflow_dispatch`）：

```bash
docker pull ghcr.io/cqut-openproject/cqut-auth:latest
```

例如 `v1.2.3` 会生成 `latest`、`v1.2.3`、`1.2.3`、`1.2`、`1` 和 `sha-<commit>`
标签。如果镜像包未设置为公开，拉取前需要先使用具有 `read:packages` 权限的
GitHub Token 登录 `ghcr.io`。

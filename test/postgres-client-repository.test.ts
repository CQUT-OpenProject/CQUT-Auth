...
(databaseUrl ? describe.sequential : describe.skip)(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      schema = schemaWithScheduler.split(
    ...
    test("rejects client creation after its maintainer is concurrently removed", async () => {
      const { repository, projects, service } = await reset();
      await pool.query(
        `insert into project_members (project_id, subject_id, role)
             values ($1, $2, 'maintainer')`,
    ...
    test("rejects secret rotation after its maintainer is concurrently removed", async () => {
      const { repository, projects, service } = await reset();
      ...
      assert.equal(
        (
          await repository.listOidcClientAuditLogs(created.client.clientId)
        ).filter((audit) => audit.action === "client.secret_rotated").length,
        0,
      );
    });

    test("rechecks active subject status inside the member transaction", async () => {
      await reset();
      const projects = new ProjectRepositoryImpl(
        () => pool,
        async () => true,
    ...
    test("serializes active project quota by creating subject", async () => {
    ...
    test("enforces subject client quota across concurrent projects", async () => {
    ...
    test("initializes the fresh schema and partial index", async () => {
      await reset();
    ...
    test("activates concurrent revision saves with latest winner", async () => {
      ...
      const current = await repository.findManagedOidcClient(active.clientId);
    ...
    test("keeps revision save and disable atomic when racing", async () => {
      const { repository, service } = await reset();
      const active = await createActive(service);
      const results = await Promise.allSettled([
        service.saveRevision(owner, projectId, active.clientId, {
    ...
    test("serializes secret rotation and isolates client authorization revocation", async () => {
      const { repository, service } = await reset();
      const created = await service.create(owner, projectId, webInput);
      ...
      assert.equal(
        current.secrets.filter((secret) => secret.status !== "revoked").length,
        2,
      );
    ...
    test("uses PostgreSQL time for rotation creation and grace expiry", async () => {
      ...
      assert.ok(createdAt >= before - 2_000 && createdAt <= Date.now() + 2_000);
      const retiring = rotated.client.secrets.find(
        (secret) => secret.status === "retiring",
      );
      assert.ok(retiring?.expiresAt);
      assert.ok(
        Math.abs(new Date(retiring.expiresAt).getTime() - createdAt - 60_000) <
          2_000,
      );
    });

    test("stacks successive approved revisions", async () => {
      const { service } = await reset();
      const active = await createActive(service);
    ...
    test("rolls back revision activation when audit insertion fails", async () => {
      const { repository, service } = await reset();
      const active = await createActive(service);
    ...
    test("serializes project member roles and preserves an owner", async () => {
      const { projects } = await reset();
      const now = new Date().toISOString();
      const audit = (
      ...
      const version = added.status === "updated" ? added.project.version : 0;
